...
contract HopeChef is OwnableUpgradeSafe {
    ...
    modifier checkHalving() {
        if (halvingChecked) {
            ...
            if (now >= nextHalvingTime) {
                massUpdatePools();
                rewardPerSecond = rewardPerSecond.mul(rewardHalvingRate).div(10000); // x96% (4% decreased every 4-weeks)
    ...
    function initialize(address _hope, uint256 _rewardPerSecond, uint256 _startTime) public initializer {
