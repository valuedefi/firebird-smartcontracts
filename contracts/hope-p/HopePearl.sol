...
contract HopePearl is ERC777OwnableUpgradeSafe {
    uint256 public cap;
    ...
    function initialize(uint256 _cap) public initializer {
        __ERC777_init("Firebird.Finance HOPE PEARL", "HOPE-P");
        cap = _cap;
        burnRate = 0; // 0%
    ...
    function mint(address _recipient, uint256 _amount) public onlyMinter {
        minterCap[_msgSender()] = minterCap[_msgSender()].sub(_amount, "HOPE-P: minting amount exceeds minter cap");
    ...
    function burnFrom(address _account, uint256 _amount) external {
        _approve(_account, _msgSender(), allowance(_account, _msgSender()).sub(_amount, "HOPE-P: burn amount exceeds allowance"));
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        require(recipient != address(0), "HOPE-P: transfer to the zero address");
    ...
    function transferFrom(address holder, address recipient, uint256 amount) public override returns (bool) {
        require(recipient != address(0), "HOPE-P: transfer to the zero address");
        require(holder != address(0), "HOPE-P: transfer from the zero address");
        ...
        _approve(holder, spender, allowance(holder, spender).sub(amount, "HOPE-P: transfer amount exceeds allowance"));
    ...
    function _send(address from, address to, uint256 amount, bytes memory userData, bytes memory operatorData, bool requireReceptionAck) internal override {
        require(from != address(0), "HOPE-P: send from the zero address");
        require(to != address(0), "HOPE-P: send to the zero address");
    ...
    function _move(address _operator, address from, address to, uint256 amount, bytes memory userData, bytes memory operatorData) internal override returns (uint256 _amountSent) {
        ...
        _balances[from] = _balances[from].sub(_amount, "HOPE-P: transfer amount exceeds balance");
